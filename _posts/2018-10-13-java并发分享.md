...
构造不可变类完成，是不是我们就能多线程下高枕无忧的去并发访问该类的对象了呢。其实还有一个问题，我们要注意到不可变类是在初始化完成后才不可变的，而初始化时是在改变的，之前讨论对象的发布与逸出时已经说过对象在初始化时多线程可见性与有序性的问题，因此只有在正确构造不可变类，并安全的发布类对象，我们才能在多线程中无需同步的访问
